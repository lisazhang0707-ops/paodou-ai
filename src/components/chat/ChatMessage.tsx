  ...
  return (
    <div className="my-3 rounded-xl overflow-hidden border border-slate-200">
      <img
        ...
        loading="lazy"
      />
      <p className="text-xs text-slate-400 text-center py-1 bg-slate-800">
        配图：{query}
      </p>
    </div>
  );
...
const markdownComponents = {
  ...
  code: ({
    ...
    return inline ? (
      <code className="bg-slate-700 text-slate-200 px-1 py-0.5 rounded text-sm">{content}</code>
    ) : (
      <pre className="bg-slate-800 p-3 rounded-xl overflow-x-auto text-sm">
        <code>{content}</code>
      </pre>
    );
  },
...
function ChatMessage({ role, content, isStreaming }: Props) {
  ...
  return (
    <div className="flex mb-4 group">
      <div className="max-w-[85%] bg-slate-800 rounded-2xl rounded-bl-md px-4 py-3 relative">
        {content ? (
        ...
        {content && !isStreaming && (
          <button
            onClick={handleCopy}
            className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity text-xs text-slate-400 hover:text-slate-200 bg-slate-700 border border-slate-600 rounded-md px-2 py-0.5"
          >
            复制
          </button>
        )}
