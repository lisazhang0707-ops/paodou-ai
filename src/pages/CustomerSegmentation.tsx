...
export default function CustomerSegmentation() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-16">
      ...
      <p className="text-[#8a827c] mb-10">基于 RFM 模型（最近购买时间、购买频率、消费金额）自动给客户打分分层</p>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="pb-3 text-sm font-medium text-[#8a827c]">客户</th>
              <th className="pb-3 text-sm font-medium text-[#8a827c]">最近购买 (天)</th>
              <th className="pb-3 text-sm font-medium text-[#8a827c]">购买频次</th>
              <th className="pb-3 text-sm font-medium text-[#8a827c]">消费金额 (¥)</th>
              <th className="pb-3 text-sm font-medium text-[#8a827c]">分层结果</th>
            </tr>
          </thead>
          <tbody>
            {customers.map((c) => (
              <tr key={c.id} className="border-b border-slate-50">
                <td className="py-3 font-medium text-[#3d3835]">{c.name}</td>
                <td className="py-3 text-[#8a827c]">{c.recency} 天</td>
                <td className="py-3 text-[#8a827c]">{c.frequency} 次</td>
                <td className="py-3 text-[#8a827c]">¥{c.monetary.toLocaleString()}</td>
                <td className="py-3">
      ...
      <div className="mt-8 p-4 rounded-xl bg-[#f5f0ea] text-sm text-[#8a827c]">
        <p className="font-medium text-[#6b6560] mb-1">分层逻辑说明</p>
