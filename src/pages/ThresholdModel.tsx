...
export default function ThresholdModel() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-16">
      <h1 className="text-3xl font-black text-slate-100 mb-2">阈值决策模型 — 线索筛选临界点</h1>
      ...
      <div className="mb-8 p-4 rounded-xl bg-slate-800 text-sm text-slate-400">
        <span className="font-medium text-slate-300">模型来源：</span>《模型思维》第 19 章 — 基于阈值的模型。存在一个临界点，超过它系统行为会发生根本变化。在营销中，这对应"线索评分设为多少分时跟进效率最高"。
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-8 mb-10">
        ...
        <div>
          ...
          <div className="text-center p-4 rounded-xl bg-blue-500/10 mb-4">
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="p-2 rounded-lg bg-slate-800">
              <p className="font-bold text-slate-300">{best.deals}</p>
              <p className="text-slate-400">成交数</p>
            </div>
            <div className="p-2 rounded-lg bg-slate-800">
              <p className="font-bold text-slate-300">¥{(best.revenue).toLocaleString()}</p>
              <p className="text-slate-400">总收入</p>
            </div>
            <div className="p-2 rounded-lg bg-slate-800">
              <p className="font-bold text-slate-300">¥{(best.cost).toLocaleString()}</p>
