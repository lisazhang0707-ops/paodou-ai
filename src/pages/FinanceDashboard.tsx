  ...
  return (
    <div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <h1 className="text-2xl font-black text-[#3d3835] mb-1">财务健康</h1>
        <p className="text-sm text-[#8a827c] mb-8">收入/成本/利润趋势 · 本量利分析 · 杜邦拆解</p>
        ...
        {chartData.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            <div className="p-5 rounded-2xl border border-[#e8e3dc] bg-white">
              <h3 className="text-sm font-bold text-[#6b6560] mb-4">收入 / 成本 / 利润趋势</h3>
            ...
            <div className="p-5 rounded-2xl border border-[#e8e3dc] bg-white">
              <h3 className="text-sm font-bold text-[#6b6560] mb-4">预算 vs 实际</h3>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={chartData}>
                  ...
                  <Bar dataKey="budgetRevenue" name="预算收入" fill="#d6d0c8" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        {cvp && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
            {[
              ...
              <div key={item.label} className="p-4 rounded-2xl border border-blue-500/20 bg-[#c2785e]/5">
                <div className="text-xs text-[#c2785e] mb-1">{item.label}</div>
        ...
        <div className="p-5 rounded-2xl border border-[#e8e3dc] bg-white mb-8">
          <h3 className="text-sm font-bold text-[#6b6560] mb-4">{editing ? "编辑记录" : "添加记录"}</h3>
          ...
          <div className="flex items-center gap-2">
            ...
            {editing && (
              <button onClick={() => { setForm(emptyRecord()); setEditing(false); }} className="px-4 py-2 text-sm font-medium text-[#8a827c] bg-[#f5f0ea] hover:bg-[#f0ebe4] rounded-xl transition-colors">
                取消
              </button>
            )}
            <label className="px-4 py-2 text-sm font-medium text-[#8a827c] bg-[#f5f0ea] hover:bg-[#f0ebe4] rounded-xl transition-colors cursor-pointer">
              CSV 导入
        ...
        {records.length > 0 && (
          <div className="overflow-x-auto rounded-2xl border border-[#e8e3dc]">
            <table className="w-full text-sm">
              ...
              <tbody>
                {records.map((r) => (
                  <tr key={r.period} className="border-t border-[#e8e3dc] hover:bg-[#f5f0ea]">
                    <td className="px-4 py-2 font-medium text-[#3d3835]">{r.period}</td>
                    ...
                    <td className="px-4 py-2">
                      <button onClick={() => handleEdit(r)} className="text-[#c2785e] hover:underline mr-2">编辑</button>
