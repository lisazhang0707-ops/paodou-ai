...
export default function DiffusionModel() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-16">
      <h1 className="text-3xl font-black text-[#3d3835] mb-2">营销传播扩散模型</h1>
      <p className="text-[#8a827c] mb-4">基于 SIR 传染病模型，模拟营销信息在目标人群中的传播过程</p>
      <div className="mb-8 p-4 rounded-xl bg-[#f5f0ea] text-sm text-[#8a827c]">
        <span className="font-medium text-[#6b6560]">模型来源：</span>《模型思维》第 11 章 — 广播模型、扩散模型和传染模型。R₀ = 接触率 × 转化率 × 传播周期。
      </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="p-3 rounded-xl bg-[#f5f0ea] text-center">
              <p className={`text-xl font-black ${data.r0 >= 1 ? "text-red-500" : "text-emerald-600"}`}>{data.r0.toFixed(2)}</p>
              <p className="text-xs text-[#8a827c]">R₀ (基本再生数)</p>
              <p className="text-xs text-[#b8b0a8] mt-0.5">{data.r0 >= 1 ? "会扩散" : "不会扩散"}</p>
            </div>
            <div className="p-3 rounded-xl bg-[#f5f0ea] text-center">
              <p className="text-xl font-black text-[#3d3835]">第{data.peakDay}天</p>
              <p className="text-xs text-[#8a827c]">感染峰值</p>
            </div>
            <div className="p-3 rounded-xl bg-[#f5f0ea] text-center">
              <p className="text-xl font-black text-[#c2785e]">{Math.round(data.totalReached).toLocaleString()}</p>
              <p className="text-xs text-[#8a827c]">总触达人数</p>
            </div>
          </div>
        </div>
      </div>
