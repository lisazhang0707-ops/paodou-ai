    ...
    <div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-black text-[#3d3835]">销售管理仪表盘</h1>
            <p className="text-sm text-[#8a827c] mt-1">平衡计分卡 · 四维度综合概览</p>
          </div>
          <button
            onClick={() => {
            ...
            className="px-4 py-2 text-sm font-medium text-[#c2785e] bg-[#c2785e]/10 hover:bg-[#c2785e]/15 rounded-xl transition-colors"
          >
            AI 策略建议
          </button>
        </div>
        ...
        {/* 预警 */}
        {alerts.length > 0 && (
          <section className="mb-8 p-5 rounded-2xl border border-red-200 bg-red-50/50">
            <h2 className="text-sm font-bold text-red-500 mb-2">预警（{alerts.length}项）</h2>
        ...
        {/* 模块入口 */}
        <section>
          <h2 className="text-sm font-bold text-[#b8b0a8] uppercase tracking-wide mb-4">分析模块</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {moduleEntries.map((m) => (
              m.soon ? (
                <div key={m.to} className="p-5 rounded-2xl border border-dashed border-[#e8e3dc] bg-[#f5f0ea]/50 opacity-60">
                  <div className={`w-10 h-10 rounded-xl ${m.color} mb-3`} />
                  <h3 className="font-bold text-[#b8b0a8] mb-1">{m.title}</h3>
                  <p className="text-xs text-[#b8b0a8] leading-relaxed">{m.desc}</p>
                  <span className="inline-block mt-2 text-[10px] text-[#b8b0a8] bg-[#f5f0ea] px-2 py-0.5 rounded">即将上线</span>
                </div>
              ) : (
                <Link
                  key={m.to}
                  to={m.to}
                  className="group p-5 rounded-2xl border border-[#e8e3dc] bg-white hover:border-[#d6d0c8] hover:shadow-md transition-all"
                >
                  <div className={`w-10 h-10 rounded-xl ${m.color} mb-3`} />
                  <h3 className="font-bold text-[#3d3835] group-hover:text-[#c2785e] transition-colors mb-1">{m.title}</h3>
                  <p className="text-xs text-[#8a827c] leading-relaxed">{m.desc}</p>
                </Link>
              )
            ))}
          </div>
